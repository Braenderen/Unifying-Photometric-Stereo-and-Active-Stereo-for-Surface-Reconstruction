...
if len(sys.argv) > 1:
    ...
    configPath = "./test/configs/BuddhaDome.yaml"
    #configPath = "./test/configs/Aplastic_15lights.yaml"
    configPath = "./test/configs/difuse_15lights.yaml"
    configPath = "./test/configs/Adifuse_15lights.yaml"
    configPath = "./test/configs/plastic_3lights.yaml"
    configPath = "./test/configs/BuddhaJune2.yaml"
...
if __name__ == "__main__":
    ...
    ############################
    #methods = [RPS.L2_SOLVER, RPS.RPCA_SOLVER, RPS.L2_NEAR_SOLVER, RPS.SBL_SOLVER_MULTICORE]
    #methods = [RPS.L2_NEAR_SOLVER]
    #methods = [RPS.L2_SOLVER, RPS.RPCA_SOLVER,RPS.SBL_SOLVER_MULTICORE,RPS.L2_NEAR_SOLVER]
    #methods = [RPS.L1_SOLVER_MULTICORE]
    #methods = [RPS.RPCA_NEAR_SOLVER]
    #methods = [RPS.RPCA_NEAR_SOLVER]
    #methods = [RPS.L2_SOLVER, RPS.L1_SOLVER_MULTICORE, RPS.SBL_SOLVER_MULTICORE, RPS.RPCA_SOLVER, RPS.L2_NEAR_SOLVER]
    methods = [RPS.L2_SOLVER,RPS.RPCA_SOLVER,RPS.L2_NEAR_SOLVER, RPS.RPCA_NEAR_SOLVER]
    #methods = [RPS.L2_NEAR_SOLVER, RPS.RPCA_NEAR_SOLVER]
    #methods = [RPS.RPCA_NEAR_SOLVER]
    #methods = [RPS.L2_NEAR_SOLVER]
    for method in methods:
        ps.METHOD = method  
        if ps.METHOD == RPS.L2_NEAR_SOLVER:
            if not exists(resultsPath + "Near_est_normal.npy") or not USE_CACHED_DATA or True:
                ps.solvePS()
            ...
            if not exists(resultsPath + "NearRPCA_est_normal.npy") or not USE_CACHED_DATA or True:
                ps.solvePS()
            ...
            if not exists(resultsPath + "LS_est_normal.npy") or not USE_CACHED_DATA or True:
                ps.solvePS()
            ...
            if not exists(resultsPath + "RPCA_est_normal.npy") or not USE_CACHED_DATA or True:
                ps.solvePS()
